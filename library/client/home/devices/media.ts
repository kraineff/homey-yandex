...
export class YandexMediaDevice extends EventEmitter {
	...
	constructor(
		...
		this.#websocket = new ReconnectSocket({
			address: async () => {
				...
				this.#conversationToken = await this.#api.quasar.getGlagolToken(
					glagolId,
					quasarInfo.platform,
				);
	...
	async #commandQuasar(instance: string, value: unknown) {
		...
		await this.#api.quasar.runDeviceAction(this.id, [action]);
		return "";
	}
	...
	async #command(
		quasar?: [string, string?],
		glagol?: [string, Record<string, unknown>?],
		volume?: number,
	) {
		...
		if (
			(this.#connection === Connection.Cloud && quasar) ||
	...
	async play() {
		const isSame = this.state.playing === true;
		if (this.#connection === Connection.Local && isSame) return;
	...
	async pause() {
		const isSame = this.state.playing === false;
		if (this.#connection === Connection.Local && isSame) return;
		if (this.#connection === Connection.Cloud) {
	...
	async rewind(position: number) {
		const isSame = this.state.playerState?.progress === position;
		if (this.#connection === Connection.Local && isSame) return;
	...
	async shuffle(enable: boolean) {
		const isSame = this.state.playerState?.entityInfo.shuffled === enable;
		if (this.#connection === Connection.Local && isSame) return;
	...
	async repeat(mode: RepeatMode) {
		const isSame = this.state.playerState?.entityInfo.repeatMode === RepeatMode[mode];
		if (this.#connection === Connection.Local && isSame) return;
		if (this.#connection === Connection.Cloud && mode === RepeatMode.None) return await this.next();
	...
	async volumeSet(value: number) {
		...
		if (this.#connection === Connection.Local && this.state.volume === volume) return;
		if (this.#connection === Connection.Cloud) {
